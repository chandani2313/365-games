<template>
    ...
    <v-stepper-header>
      <v-stepper-step color="#1e0741" :complete="e1 > 1" step="1">Step 1</v-stepper-step>

      <v-divider></v-divider>

      <v-stepper-step color="#1e0741"  :complete="e1 > 2" step="2">Step 2</v-stepper-step>

      <v-divider></v-divider>

      <v-stepper-step color="#1e0741" step="3">Step 3</v-stepper-step>
      <v-divider></v-divider>
       <v-stepper-step color="#1e0741" step="4">Step 4</v-stepper-step>
    </v-stepper-header>
    ...
    <v-stepper-items>
      <v-stepper-content step="1">
           <div class="stepper">
             ...
             <div class="time">{{changeDate(selectedTournament.expiryDate)}}</div>
             <div class="buttons">
  ...
  <div class="stepper">
    <h3>Pay with Gem</h3>
    <div class="gems">
      <div class="current_gem">Available Gem: {{selectedTournament.availableGems}}G</div>
       <div class="current_gem">Entry Fee Gem: {{selectedTournament.entryFeeGem}}G</div>
    </div>
    <div class="para" v-if="selectedTournament.entryFeeGem > selectedTournament.availableGems">Please purchase Gem to continue</div> 
    <div class="para" v-else>You have enough gem to continue</div> 
    <div class="pur_con">
      <div class="purchase" v-if="selectedTournament.entryFeeGem > selectedTournament.availableGems" @click="goToMarketPage">Purchase</div>
      <div class="purchase" v-else @click="e1 = 4">Continue</div>
    </div>
  </div>
        <v-btn
         dark
      ...
      <div class="stepper">
        ...
        <div class="agree-btn">
          <button @click="joinTournament">Agree and Join</button>
        </div>
      </div>
        <v-btn text @click="closeStepper">Cancel</v-btn>
      </v-stepper-content>
     ...
     join() {
      axios
        ...
        .then((response) => {
          ...
          this.snackbarText = 'Joined tournament successfully';
              this.color = "green";
              this.snackbar = true;
              this.timeout = 3000;
          this.closeStepper();
    ...
    updateGameDetails(game, gameId, gameIgn) {
      ...
      axios.post('/user/update/game-id', { data })
        ...
        .catch((e) => {
          if (e.response.status === 400) {
           console.log('err', e.response.data)
            const gameData = e.response.data.message;
              this.snackbarText = gameData;
              this.color = "red darken-4";
              this.snackbar = true;
              this.timeout = 3000;
          }
